...
```{r}
# You should know what subsets are and what they do.
?subset
# I wonder why the convention for data is x. 
...
```{r}
?dim # Woah cool, dimensions of an object.
...
```{r}
# mpg smaller than 14 or dis bigger than 390
...
```{r}
?mtcars
lightCars <- subset(mtcars, wt < 2.0) # Didn't know 1 pound = 1000 lb.
lightCars
```
...
# The above line makes an array? I think. 1973 1974 1973 1974 ...
?c # Combine values into a vector or list. Wonder what vectors and list have in difference in R.
```
...
```{r}
mtcars
# I think you should find out about mutable, and immutable functions.
subset(mtcars, select = -year) # Didn't know the subset does a mutation on the main list.
```
...
mtcars$weight_class
# So, if you want a three-way value, that means you have to convert the list two times?
# I think there will be a better way in the R language. I hope so.
```
...
```{r}
# Woah, you actually have to think about the working memory. 
# Well, this will help me at least think about memory! Finally.
# Never got the chance to actually 'think and care' about memory because of the GC.
rm(cond)
rm(efficient)
```
